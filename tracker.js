(function () {
    ...
    // Function to Get User ID (No Guest Logic)
    function getUserId() {
    ...
    // Send Tracking Data (Only if userId is available)
    function sendTrackingData(eventType, extraData = {}) {
        const userId = getUserId();
        if (!userId) return; // Stop tracking if no user is logged in
    ...
    // Track Login Event (Call this function after successful login)
    function trackLogin(userId, email, role) {
    ...
    // Track Events (Only for logged-in users)
    document.addEventListener("click", function (event) {
        if (!getUserId()) return; // Stop tracking if user is not logged in
    ...
    // Expose Login & Logout tracking functions to be used in login/logout actions
